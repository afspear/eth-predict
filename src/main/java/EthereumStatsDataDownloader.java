...
public class EthereumStatsDataDownloader extends DataDownloader {
    private String csv = "?output=csv";
    private String allDataFileName = "allEhereumStatsData.csv";
    ...
    public static void main(String[] args) {
        ...
        EthereumStatsDataDownloader ethereumStatsDataDownloader = new EthereumStatsDataDownloader();
        ...
        try {
            ...
            urlsList
                    ...
                    .forEach(s -> {
                        ...
                        try {
                            String dataCSV = ethereumStatsDataDownloader.download(s);
                            URI uri = URI.create(s);
                            String path = uri.getPath();
                            String dataName = path.substring(path.lastIndexOf('/') + 1);
                            ethereumStatsDataDownloader.combineCSVData(dataCSV, dataName);
                        } catch (Exception e) {
                            e.printStackTrace();
                        }
            ...
            Gson gson = new Gson();
            Files.write(Paths.get("allData.json"), gson.toJson(ethereumStatsDataDownloader.allData).getBytes());


            ethereumStatsDataDownloader.writeAllDataAsCSV();
